...
/*
The RUtil namespace provides numerous useful functions which act primarily on TGraphs. 
...
can be extended to use other 'vector' features in the future. there is certainly a smarter way to do this, but i'm not a programmer.
  ...
  */
  ...
  private:
    std::vector<RUtil::TVec1D<T>> vec;
  public:
    TVec2D(){
      vec=std::vector<RUtil::TVec1D<T>>();
    }
     TVec2D(int N){
       vec=std::vector<RUtil::TVec1D<T>>(N);
    }
     TVec2D(int N, int M){
       vec=std::vector<RUtil::TVec1D<T>>(N, RUtil::TVec1D<T>(M));
     }
     TVec2D(int N, int M, T const & init){
       vec=std::vector<RUtil::TVec1D<T>>(N, RUtil::TVec1D<T>(M, init));
    }
     
    RUtil::TVec1D<T> & operator[](const int index);
  ...
  template <class T> RUtil::TVec1D<T> & TVec2D<T>::operator[](const int index){
    return vec[index];
  }
...
utilities.
  ...
  ***********************************************/
  //return the power dB level relative to ref
  double toDBPower(double val, double ref);
  //return the field dB level relative to ref
  double toDBField(double val, double ref);
  //volts to dbm/hz
...
the SVD namespace, which has useful utilities for SVD filtration methods
  ...
  ******************************************************/
  ...
  namespace retcr{
    ...
    void checkCausality(double l0Fix[10], UInt_t panelsInTrigger[10],int aCausal[10]);
    double maxTheta(int aCausal[10], double l0Fix[10]);
    ...
    TGraph * removeFreqsNew(TGraph *ingr,double tx_freq, double TxOn);
